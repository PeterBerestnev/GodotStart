...
var rest_material = load("res://Enemy/EnemyRestMaterial.tres")
		...
		match current_state:
			state.SEEKING:
				if current_node < path.size():
					...
					if seeking_vector.length() < 2:
						current_node += 1
					#Check if we are close to the player
					#If close - attack
...
#				print("i hit you, u got: ", player_stats.current_HP , '/', player_stats.max_HP)
				current_state = state.RETURNING
				attack_target = return_target
			state.RETURNING:
				current_state = state.RESTING
				$MeshInstance.set_surface_material(0, rest_material)
